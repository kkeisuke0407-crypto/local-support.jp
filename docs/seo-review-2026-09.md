...
## 9. Google で上がらない原因（2026-09-25 追記）

GSC「リンク」の外部リンクをエクスポートしたところ **0件**。Google が把握している被リンクはゼロ。

1. **外部評価ゼロ**：開設4か月・被リンク0。順位10位前後で止まる主因
2. **量産ページ比率**：インデックス対象293ページの58%（171）が県別LP。登録数は減少が続く。インデックス対象ページから出る内部リンクの33%が noindex の41県ページ行き
3. **一次情報の不足**：コラム81本の中心は相場・法令の一般解説で競合と同質。CVの出た消防のコラム10本に対し害虫は20本

打ち手（効果順）：①被リンク獲得の手段を検討（プレスリリース、掲載業者バナーなど）②10月下旬の判定後に県別LPを整理、41県リンクの削減（全ページに効くので要確認）③消防クラスターを20〜30本に

- 🚫 打診の返信率・紹介可の社数・1件あたりの紹介社数など、運営の内部データは公開コンテンツに使わない（2026-09-25 ユーザー判断。作成した記事は公開前に削除）
- 2026-09-25：/column/shobo-setsubi-gimu-check/（消防点検の義務チェック）を公開。用途・延べ面積・階段・収容人員・CO2消火設備から、消防設備点検と防火対象物点検の要否・報告頻度・資格者の要否を判定。消防設備点検・防火対象物点検のLPの関連コラムに掲載
