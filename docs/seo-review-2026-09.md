...
打ち手（効果順）：①実案件データの記事化（被リンク・AI引用の受け皿）②被リンク獲得（①を材料にしたプレスリリース、掲載業者バナー）③10月下旬の判定後に県別LPを整理、41県リンクの削減（全ページに効くので要確認）④消防クラスターを20〜30本に

- 2026-09-25：①の第1弾 `/column/gyosha-henshin-data/`（業者266社への問い合わせの返信率・返信日数・サービス別・連絡手段別）。Dataset 構造化データと「引用自由（リンク明記）」を付けた。数字は台帳 `sheet-tab2-quotes.csv` から集計、四半期ごとに更新する
