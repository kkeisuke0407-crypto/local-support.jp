...
def main():
    ...
    for r in quotes:
        t = tally.setdefault(r[0], {'打診': 0, '紹介可': 0, '辞退': 0, '不達': 0,
                                    '未依頼': 0, '回答待ち': 0, '共有済': 0})
        adopt = r[C2['採用可否']]
        if adopt in ('採用', '成約') or adopt.startswith('★依頼者が選定') or '連絡先共有' in adopt:
            t['共有済'] += 1
        s = r[C2['見積ステータス']]
        if s in ('未依頼', '下書き済', '送信準備中', '送信不可'):
            t['未依頼'] += 1
            continue
        if s in ('対象外', '送信保留'):
            continue
    ...
    for r in cases:
        ...
        if closed:
            mark, rank, act = '✅ 決着', 7, '—'
        elif '追加打診中' in status:
            # お繋ぎ後に一部区分の業者が抜け、その区分だけ2社目を探し直している案件。
            # 共有済の業者がいても「結果確認」ではなく打診が要る
            mark, rank = '🟡 追加打診中', 2
            act = '抜けた区分の2社目を取る（詳細ステータス参照）'
        elif '比較ページ再送' in status:
            # お繋ぎ済みの区分とは別に、抜けた区分だけ比較ページを送り直した案件。
            # 共有済の業者がいても、いまは依頼者の選定待ち
            mark, rank, act = '📤 選定待ち', 5, '依頼者の選定を待つ（再送分）'
        elif t['共有済'] or '連絡先共有' in status:
            # ⑩まで進んだ案件。運営の手離れ済みで、あとは結果の確認だけ
            mark, rank = '🤝 お繋ぎ済み', 6
            act = '結果を確認する（⑩から1週間が目安）'
        elif ('比較ページ' in status and '送付' in status) or '提示' in status:
            mark, rank, act = '📤 選定待ち', 5, '依頼者の選定を待つ'
        elif '紹介先なし' in status:
            # 紹介可の業者はいるが、依頼者が自力で見つけていた等で仲介の余地がない案件。
            # 打診を増やすかクローズするかを依頼者に確認する段階
            mark, rank = '⚠️ 紹介先なし', 1
            act = '他社を当たるか終了するかを依頼者に確認する'
        elif '依頼者へ確認中' in status:
            # 紹介可が1社のまま時間が経ち、1社でのご紹介可否を依頼者に尋ねた案件。
            # 打診を増やす段階ではなく、依頼者の返答を待つ段階
            mark, rank = '📨 依頼者の返答待ち', 4
            act = '1社でのご紹介可否について依頼者の返信を待つ'
        elif t['打診'] == 0:
            mark, rank = '🚨 未打診', 0
