import type { PrefRef } from './types.ts';

/** JIS都道府県コード（全国対応のための母集団拡張用） */
...
export function prefByName(name: string): PrefRef {
  const code = PREF_CODES[name];
  if (!code) throw new Error(`未知の都道府県: ${name}`);
  return { name, code };
}

export const TOKYO: PrefRef = { name: '東京都', code: '13' };
