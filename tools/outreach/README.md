...
> ## 🛑 2026-07-28 停止中
> 反響ゼロのため冷メール送信を停止。`send.js` にガードを入れてあり、
> `.env` に `OUTREACH_ENABLED=true` が無い限り本番送信は起動しない（dry-runは可）。
> **別環境（PC等）で古いコピーが回っている場合はこのガードが効かない。**
> その場合の確実な停止手段は Resend APIキーの失効：
> 1. Resend で新しいAPIキーを発行
> 2. Cloudflare Pages の環境変数 `RESEND_API_KEY` を新キーに差し替え → 再デプロイ
>    （※これをやらないと問い合わせフォーム・ヒアリング・業者選定の通知メールが止まる）
> 3. 旧キーを Resend で **Revoke** → 旧キーを持つ送信スクリプトは全て 401 で停止
